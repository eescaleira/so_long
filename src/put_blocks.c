...
/*   Updated: 2023/12/16 16:52:51 by eescalei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_put_image_to_window(data->mlx, data->mlx_win, data->imgs->wall->img, x * 32, y * 32);
}
...
{
	mlx_put_image_to_window(data->mlx, data->mlx_win, data->imgs->blocked_exit->img, x * 32, y * 32);
}

void put_collectible(t_mlx_data *data, int x, int y)
{
	mlx_put_image_to_window(data->mlx, data->mlx_win, data->imgs->collectible->img, x * 32, y * 32);
}

void put_player(t_mlx_data *data, int x, int y)
{
	mlx_put_image_to_window(data->mlx, data->mlx_win, data->imgs->player->img, x * 32, y * 32);
}
